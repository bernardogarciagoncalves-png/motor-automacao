...
if (process.env.SUPABASE_URL && (process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_ANON_KEY)) {
  try {
    ...
    console.log('Aviso: Supabase não inicializado:', e.message);
  }
}
...
app.post('/api/executar-tarefa', async (req, res) => {
  try {
    ...
    // Filtro de Categorias (Moradia x Comercial)
    ...
    if (Array.isArray(contextoImoveis) && contextoImoveis.length > 0) {
      ...
      if (pedeResidencial && !pedeComercial) {
        contextoImoveis = contextoImoveis.filter(imovel => {
          ...
          return !palComerciais.some(c => dadosStr.includes(c));
        });
      }
    ...
    // Leitura do Histórico de Aprendizado
    ...
    if (supabase) {
      try {
        ...
        console.log('Aviso: Tabela historico_aprendizado indisponível no momento:', errSupabase.message);
      }
    }
...
REGRA RIGOROSA DE CARDS DE IMÓVEIS:
